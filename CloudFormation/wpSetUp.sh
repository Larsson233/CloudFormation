...
/tmp/wp-cli.phar config create --dbname=!GetAtt MyDBInstance.DBName  --dbuser=Admin --dbpass=Password --path=/var/www/html

TOKEN=$(curl -X PUT "http://169.254.169.254/latest/api/token" -H "X-aws-ec2-metadata-token-ttl-seconds: 21600")
DNS=$(curl -H "X-aws-ec2-metadata-token: $TOKEN" -v http://169.254.169.254/latest/meta-data/public-hostname)

/tmp/wp-cli.phar core install --url=$DNS --title=Example --admin_user=Admin --admin_password=Password --admin_email=info@example.com --path=/var/www/html
